...
export async function getAllChainBalances() {
  try {
    ...
    async function getChainBalanceAndAddress(chain: Chain) {
      try {
        ...
        if (balances && balances.length > 0) {
          ...
          console.log(
            `✅ ${balances.length} balances trouvées pour ${chain} (${nonZeroBalances.length} non vides)`,
          );
    ...
    console.log(
      `Nombre de chaînes avec des balances non vides: ${Object.keys(chainesAvecBalances).length}`,
    );
