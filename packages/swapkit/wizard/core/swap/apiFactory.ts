...
export class EvmApiFactory {
  ...
  createApi(chain: Chain, apiType: ApiType = ApiType.ALCHEMY) {
    const type = apiType || this.defaultApiType;
    const chainId = ChainToChainId[chain];

    switch (type) {
      case ApiType.ALCHEMY:
        return alchemyApi({
          apiKey: this.apiKeys.alchemy,
          chainId,
        });
      case ApiType.COVALENT:
        return covalentApi({
          apiKey: this.apiKeys.covalent,
          chainId,
        });
      case ApiType.ETHPLORER:
        return ethplorerApi(this.apiKeys.ethplorer);
      default:
  ...
  ): Record<Chain, ReturnType<typeof alchemyApi> | ReturnType<typeof covalentApi> | ReturnType<typeof ethplorerApi>> {
    return chains.reduce((apis, chain) => {
