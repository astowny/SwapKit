...
async function getAvailableProviders() {
  ...
  try {
    ...
    console.log(`✅ ${providers.providers.length} fournisseurs trouvés`);
    console.log("Fournisseurs disponibles:", providers.providers);
...
async function getTradingPairs(providers: ProviderName[]) {
  console.log(`Récupération des paires de trading pour les fournisseurs: ${providers.join(", ")}...`);
  ...
  try {
    ...
    if (!tradingPairs || !tradingPairs.pairs || tradingPairs.pairs.length === 0) {
      console.error(`❌ Aucune paire de trading trouvée pour les fournisseurs: ${providers.join(", ")}`);
      return null;
    }
...
async function main() {
  ...
  if (!provider) {
    console.log("Aucun fournisseur spécifié. Récupération de la liste des fournisseurs disponibles...");
